...
const StoriesCarousel = () => {
  ...
  return (
    <div className="relative bg-white rounded-xl border border-gray-200 p-4 mb-6">
      ...
      >
        ...
        {stories.map((story) => (
          <div key={story.id} className="flex flex-col items-center space-y-2 flex-shrink-0 cursor-pointer">
            <div className={`relative p-0.5 rounded-full ${story.viewed ? 'bg-gray-300' : 'bg-gradient-to-r from-purple-600 to-pink-600'}`}>
              <Avatar className="h-16 w-16 border-2 border-white">
                <AvatarImage src={story.user.avatar} />
                <AvatarFallback>{story.user.username[0]?.toUpperCase()}</AvatarFallback>
              </Avatar>
            </div>
            <span className="text-xs text-gray-600 max-w-[60px] truncate">{story.user.username}</span>
          </div>
        ))}
      </div>
    </div>
